...
PyMethodDef vectorMethods[] = {
	{"toNumpy", (PyCFunction)PyVector::toNumpy, METH_VARARGS}, {NULL} };
struct PyModuleDef vectorModule = { PyModuleDef_HEAD_INIT, "VectorModule",
								   "documentation is here", -1, vectorMethods };

PyNumberMethods vectorNumberMethods = { NULL };

static PyMethodDef matrixMethods[] = {
	{"toNumpy", (PyCFunction)PyMatrix::toNumpyMatrix, METH_VARARGS}, {NULL} };
...
PyMODINIT_FUNC PyInit_VectorModule(void) {
	import_array();
	PyObject* m;
	vectorNumberMethods.nb_add =
		(binaryfunc)PyVector::vectorSum;

	vectorNumberMethods.nb_subtract =
		(binaryfunc)PyVector::vectorSubstract;
	...
	if (PyType_Ready(&PyVector::vectorType) < 0) return NULL;
	if (PyType_Ready(&PyMatrix::matrixType) < 0) return NULL;
